...
<head>
    ...
    @if (\Request::is('/'))
        <title>{{ucwords(@$setting_data->website_name ?? 'Hanabi')}}</title>
    @else
        <title>@yield('title') | {{ucwords(@$setting_data->website_name ?? 'Hanabi')}} </title>
    @endif

    <link rel="shortcut icon" type="image/x-icon" href="{{ (@$setting_data->favicon) ? asset('/images/settings/'.@$setting_data->favicon):'' }}">

    <meta property="og:title" content=" {{ucwords(@$setting_data->meta_title ?? 'Hanabi')}}" />
    ...
    <meta property="og:description" content=" {{ucwords(@$setting_data->meta_description ?? 'Hanabi')}}" />
                                ...
                                <div class="site-branding me-auto">
                                    <h1>
                                        <a class="home-link" href="/" title="{{ $setting_data->website_name ?? 'Hanabi' }}" rel="home">
                                            <img id="logo-img" height="48" width="147" class="img-fluid auto_size" src="{{$setting_data->logo ? asset('/images/settings/'.@$setting_data->logo):''}}" alt="logo-img">
                                        </a>
                                    </h1>
